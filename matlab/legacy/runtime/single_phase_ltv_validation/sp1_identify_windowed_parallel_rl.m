function result = sp1_identify_windowed_parallel_rl(data, config, opts)
%SP1_IDENTIFY_WINDOWED_PARALLEL_RL Local LTI approximation for parallel R-L.
%
% In each window:
%
%   i ~= G_w v + Gamma_w primitive(v),   Gamma_w = 1/L_w.
%
% The primitive is computed from the local harmonic fit and intentionally
% ignores DC to avoid converting measurement offset into a ramp.
...
for w = 1:numel(starts)
    ...
    signals = struct('t', t, 'v', vFit.values{1}, ...
        'qv', vFit.primitive, 'i', iFit.values{1});
    X = [signals.v, signals.qv];
    y = signals.i;
    ...
    rows(w, :) = {w, mean(t), t(1), t(end), mat2str(harmonics), ...
        theta(1), R, theta(2), L, fit.rank, fit.conditionNumber, ...
...
windows = cell2table(rows, 'VariableNames', { ...
    ...
    'G', 'R', 'Gamma', 'L', 'Rank', 'ConditionNumber', ...
    'EquationResidual', 'PowerResidual', 'Passive', 'Valid'});

result = struct();
result.model = 'windowed_parallel_rl_lti';
